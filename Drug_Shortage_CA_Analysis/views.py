...
def update_database():
    ...
    for datum in drug_data:
        ...
        if code not in codes:
            ...
            cur.execute(
                "SELECT company_code FROM companies WHERE company_name = ?", (owner,)
            )
            ccode = cur.fetchall()[0][0]
            details = (code, datum["brand_name"], ccode, din)
            values.append(details)
    cur.executemany(
        "INSERT INTO drugs (drug_code, drug_name, owner, din) VALUES(?, ?, ?, ?)",
        values,
    )
    con.commit()
...
def get_summary(subj, type, term):
    if subj == 0:
        ...
        if reports["total"] == 0:
            ...
            if (
                reports["data"][0]["status"] == "active_confirmed"
                or reports["data"][0]["status"] == "resolved"
            ):
                dict2["report_url"] = (
                ...
                try:
                    dict2["latest_start"] = reports["data"][0][
                        "actual_start_date"
                    ].rpartition("T")[0]
                except:
                    dict2["latest_start"] = reports["data"][0][
                ...
                dict2["latest_reason"] = reports["data"][0]["shortage_reason"][
                    "en_reason"
                ]
                if reports["data"][0]["status"] == "resolved":
                    dict2["latest_resolved"] = True
                    dict2["resolved_date"] = reports["data"][0][
                        "updated_date"
                    ].rpartition("T")[0]
                    ...
                    dict2["latest_est_end"] = reports["data"][0][
                        "estimated_end_date"
                    ].rpartition("T")[0]
            elif (
                reports["data"][0]["status"] == "discontinued"
                or reports["data"][0]["status"] == "reversed"
            ):
                dict2["report_url"] = (
                ...
                dict2["status"] = reports["data"][0]["status"]
                dict2["latest_start"] = reports["data"][0]["updated_date"].rpartition(
                    "T"
                )[0]
                dict2["latest_reason"] = reports["data"][0]["discontinuance_reason"][
                    "en_reason"
                ]
                if reports["data"][0]["status"] == "reversed":
                    dict2["latest_reversed"] = True
                    dict2["reversed_date"] = reports["data"][0][
                        "updated_date"
                    ].rpartition("T")[0]
                    dict2["latest_est_end"] = "N/A"
                else:
        ...
        url = (
            ...
            + "/search?din="
            + term
            + "&orderby=updated_date&order=desc&filter_status=resolved"
        )
        response = requests.get(url, headers=header)
        ...
        dict2["location"] = (
            js["city_name"] + ", " + js["province_name"] + ", " + js["country_name"]
        )
        ...
        url = (
            base_url
            + "/search?orderby=updated_date&order=desc&filter_status=resolved&term="
            + name
        )
        header = {"auth-token": auth_token}
        response = requests.get(url, headers=header)
        ...
        if "error" in reports:
            ...
            url = (
                base_url
                + "/search?orderby=updated_date&order=desc&filter_status=active_confirmed&term="
                + name
            )
            response = requests.get(url, headers=header)
            reports = response.json()
...
def get_updates():
    ...
    for x in range(20):
        ...
        dict["url"] = "https://www.drugshortagescanada.ca/shortage/" + str(
            reports[x]["id"]
        )
...
def get_graph_all():
    ...
    for s in shortages:
        ...
        if isinstance(drug_code, str):
            drug_code = cur.execute(
                "SELECT drug_code FROM drugs WHERE din = ?", (drug_code,)
            ).fetchall()[0][0]
        company_code = s[2]
        reason = s[3]
        started = s[4]
        report_id = s[5]
        din = s[6]
        drug_name = cur.execute(
            "SELECT drug_name FROM drugs WHERE drug_code = ?", (drug_code,)
        ).fetchall()[0][0]
        company_name = cur.execute(
            "SELECT company_name FROM companies WHERE company_code = ?", (company_code,)
        ).fetchall()[0][0]
        net.add_node(
            drug_code,
            label=drug_name,
        ...
        ingredients = cur.execute(
            "SELECT ingredient_name FROM ingredients WHERE used_in = ?", (drug_code,)
        ).fetchall()
...
def get_graph_entity(subj, type, id):
    ...
    if subj == 0:
        # Drug node
        cur.execute(
            "SELECT drug_code,drug_name,owner,din,status,company_name FROM d_and_c WHERE din = ?",
            (id,),
        )
        drug_data = cur.fetchall()
        ...
        for ingredient in ingredients:
            ing_name = ingredient[0]
            ing_links = cur.execute(
                "SELECT used_in FROM ingredients WHERE ingredient_name = ?", (ing_name,)
            ).fetchall()
            for ing in ing_links:
                ...
                if drug_code not in drug_nodes:
                    ...
                    cur.execute(
                        "SELECT drug_code,drug_name,owner,din,status,company_name FROM d_and_c WHERE drug_code = ?",
                        (drug_code,),
                    )
                    drug_data = cur.fetchall()
                    drug_code = drug_data[0][0]
                    ...
                    company_name = drug_data[0][5]
                    net.add_node(
                        company, label=company_name, color="#5380cf", shape="square"
                    )
                    net.add_edge(company, drug_code)

                    ingredients = cur.execute(
                        "SELECT ingredient_name FROM ingredients WHERE used_in = ?",
                        (drug_code,),
                    ).fetchall()
                    for ingredient in ingredients:
                        ing_name = ingredient[0]
            ...
            if drug_code not in drug_nodes:
                ...
                for ingredient in ingredients:
                    ...
                    if ing_name not in ingredient_nodes:
                        ingredient_nodes.add(ing_name)
                        net.add_node(
                            ing_name, label=ing_name, color="#d0d624", shape="triangle"
                        )
                    net.add_edge(drug_code, ing_name)
    ...
    # Company
        ...
        for drug in drug_data:
            ...
            if drug_code in shortage_list:
                ...
                color = "#89d624"
            net.add_node(
                drug_code, label=drug_name, title=title, color=color, shape="diamond"
            )
            net.add_edge(company, drug_code)
    ...
    # Ingredient
        ...
        net.add_node(
            id, label=id, color="#d0d624", size=100, mass=100, shape="triangle"
        )
